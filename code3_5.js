const game = function () {
    ...
    canvas.width = 1200 ;
    canvas.height = 600 ;
    const numberOfenemiesPos = 35;//20 + 5 * level;
...
// Using Jquery for fun!
    ...
    //************************ ASSIGNING enemiesPos & Roses PROPERTIES *****************************************
    const enemiesPos = createPoints (numberOfenemiesPos, canvas.width, canvas.height);
    const roseRain = createPoints (numberOfRoses, canvas.width, canvas.height);
    ...
    const renderIfwin = function (n) {
        ...
        const renderIfwinHelper = function (i) {
            ...
            ctx.drawImage(rose, roseRain[i].x, roseRain[i].y, roseRain[i].width*3, roseRain[i].height*3);
